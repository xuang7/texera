...
export class DatasetService {
  ...
  public multipartUpload(
    ...
    return new Observable(observer => {
      ...
      const initialNumPartsToRequest = 0; // Not using lakefs to generate presigned urls
      const subscription = this.initiateMultipartUpload(ownerEmail, datasetName, filePath, initialNumPartsToRequest)
        .pipe(
          switchMap(initiateResponse => {
            const { uploadId, physicalAddress } = initiateResponse;
            if (!uploadId) {
            ...
            // 1. Create a stream of batch numbers: [1, 2, 3, ...]
            ...
            return batchNumbers$.pipe(
              concatMap(batchNum => {
                ...
                // Sign URLs for the current batch
                return this.presignPendingParts(
                  ownerEmail,
                  datasetName,
                  filePath,
                  uploadId,
                  ...
                  mergeMap(signResponse => {
                    const urls = signResponse.presignedUrls;

                    return from(Object.entries(urls)).pipe(
                      map(([partNumberStr, url]) => ({
                        partNumber: Number(partNumberStr),
                        url: url,
                      })),

                      // 2) Use mergeMap to upload chunk by chunk
  ...
  private initiateMultipartUpload(
    ...
    filePath: string,
    numParts: number
  ): Observable<{ uploadId: string; physicalAddress: string }> {
    ...
    return this.http.post<{ uploadId: string; physicalAddress: string }>(
